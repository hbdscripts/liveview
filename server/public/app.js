...
const API = '';
    ...
    (function initAdminNotesModal() {
      var btn = document.getElementById('footer-notes-btn');
      if (!btn) return;

      function ensureModal() {
        var modalEl = document.getElementById('admin-notes-modal');
        if (modalEl) return modalEl;
        var wrap = document.createElement('div');
        wrap.innerHTML =
          '<div class="modal modal-blur fade" id="admin-notes-modal" tabindex="-1" aria-hidden="true" aria-label="Admin notes">' +
            '<div class="modal-dialog modal-dialog-centered">' +
              '<div class="modal-content">' +
                '<div class="modal-header">' +
                  '<h3 class="modal-title">Notes</h3>' +
                  '<button type="button" class="btn-close" aria-label="Close" data-admin-notes-close></button>' +
                '</div>' +
                '<div class="modal-body">' +
                  '<form id="admin-notes-form" class="mb-3">' +
                    '<div class="mb-2">' +
                      '<label for="admin-notes-body" class="form-label visually-hidden">New note</label>' +
                      '<textarea class="form-control" id="admin-notes-body" name="body" rows="2" placeholder="Add a note…" maxlength="4096"></textarea>' +
                    '</div>' +
                    '<button type="submit" class="btn btn-primary btn-sm">Add</button>' +
                  '</form>' +
                  '<div id="admin-notes-list" class="small text-secondary"></div>' +
                '</div>' +
              '</div>' +
            '</div>' +
          '</div>';
      ...
      var form = document.getElementById('admin-notes-form');
      var bodyInput = document.getElementById('admin-notes-body');
      var listEl = document.getElementById('admin-notes-list');
      if (!modalEl || !form || !bodyInput || !listEl) return;

      var API_BASE = (typeof API !== 'undefined' ? API : '') || '';
      var backdropAttr = 'data-admin-notes-backdrop';
      ...
      function hideModal() {
        ...
        try {
          document.querySelectorAll('.modal-backdrop[' + backdropAttr + '="1"]').forEach(function (b) {
            try { b.remove(); } catch (_) { if (b && b.parentNode) b.parentNode.removeChild(b); }
          });
        } catch (_) {}
      }
      ...
      function showModal() {
        try {
          ...
          if (Modal) {
            var m = Modal.getOrCreateInstance(modalEl, { backdrop: true, keyboard: true });
            m.show();
            return;
          }
        } catch (_) {}
        ...
        try { modalEl.setAttribute('aria-hidden', 'false'); } catch (_) {}
      ...
      function loadNotes() {
        ...
        fetch(API_BASE + '/api/admin/notes', { credentials: 'same-origin' })
          .then(function(r) { return r.ok ? r.json() : null; })
          .then(function(data) {
            var notes = (data && Array.isArray(data.notes)) ? data.notes : [];
            if (!notes.length) { listEl.innerHTML = '<span class="text-muted">No notes yet.</span>'; return; }
            listEl.innerHTML = notes.map(function(n) {
              var body = (n.body && String(n.body).trim()) ? escapeHtml(n.body) : '\u2014';
              var at = n.created_at != null ? new Date(Number(n.created_at)).toLocaleString() : '';
              return '<div class="mb-2"><div class="text-body">' + body + '</div><div class="text-muted" style="font-size:.75rem">' + escapeHtml(at) + '</div></div>';
            }).join('');
          })
          .catch(function() { listEl.innerHTML = '<span class="text-danger">Failed to load notes.</span>'; });
      }

      function wireCloseHandlers() {
        if (!modalEl || modalEl.getAttribute('data-admin-notes-wired') === '1') return;
        modalEl.setAttribute('data-admin-notes-wired', '1');
        try {
          modalEl.addEventListener('click', function (e) {
            if (e && e.target === modalEl) hideModal();
          });
        } catch (_) {}
        try {
          var closeBtn = modalEl.querySelector('[data-admin-notes-close], [data-bs-dismiss="modal"]');
          if (closeBtn) closeBtn.addEventListener('click', function (e) { try { e.preventDefault(); } catch (_) {} hideModal(); });
        } catch (_) {}
        try {
          if (document.documentElement.getAttribute('data-admin-notes-esc') !== '1') {
            document.documentElement.setAttribute('data-admin-notes-esc', '1');
            document.addEventListener('keydown', function (e) {
              if (e && e.key === 'Escape' && (manualShown || modalEl.classList.contains('show'))) hideModal();
            });
          }
        } catch (_) {}
      }
      ...
      form.addEventListener('submit', function(e) {
        ...
        if (!body) return;
        var createdBy = '';
        try { createdBy = (window.__kexoMe && window.__kexoMe.email) ? String(window.__kexoMe.email) : ''; } catch (_) { createdBy = ''; }
        fetch(API_BASE + '/api/admin/notes', {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ body: body, created_by: createdBy || undefined })
        })
          .then(function(r) { return r.ok ? r.json() : null; })
          .then(function() { if (bodyInput) bodyInput.value = ''; loadNotes(); })
          .catch(function() {});
      });
    })();
