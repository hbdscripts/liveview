...
async function getOtherRevenueDrilldownByUtmSource(options = {}) {
  const rangeKey = normalizeRangeKey(options.rangeKey);
  const shop = options.shop != null ? String(options.shop).trim() : '';
  const utmSourceRaw = options.utmSource != null ? String(options.utmSource).trim() : '';
  const utmSourceFilter = utmSourceRaw === '' ? 'direct' : utmSourceRaw;
  const now = Date.now();
  const timeZone = store.resolveAdminTimeZone();
  const bounds = store.getRangeBounds(rangeKey, now, timeZone);
  const { purchaseFilterExcludeDuplicateH, purchaseFilterExcludeTokenWhenOrderExists } = store;
  ...
  if (adsDb && shop) {
    try {
      ...
      console.warn('[ads.other-revenue-drilldown] failed to read ads_orders_attributed (non-fatal):', e && e.message ? e.message : e);
    }
  }
  ...
  const excludeClause = excludedOrderIds.size > 0
    ...
    : '';
  const utmExpr = `COALESCE(NULLIF(TRIM(s.utm_source),''), 'direct')`;
  const variantExpr = `COALESCE(NULLIF(TRIM(s.attribution_variant),''), 'other')`;
  const baseParams = [bounds.start, bounds.end, utmSourceFilter];
  const countParams = excludeClause ? [...baseParams, ...Array.from(excludedOrderIds)] : baseParams;
  const revParams = excludeClause ? [...baseParams, ...Array.from(excludedOrderIds)] : baseParams;

  const countSql = `
    SELECT ${variantExpr} AS variant_key,
           MAX(av.label) AS variant_label,
           MAX(av.channel_key) AS channel_key,
           MAX(av.icon_spec) AS icon_spec,
           COUNT(DISTINCT p.session_id) AS sessions,
           COUNT(DISTINCT p.purchase_key) AS orders
    FROM purchases p
    INNER JOIN sessions s ON s.session_id = p.session_id
    LEFT JOIN attribution_variants av ON av.variant_key = ${variantExpr}
    WHERE p.purchased_at >= ? AND p.purchased_at < ?
      AND ${utmExpr} = ?${excludeClause}
    ${purchaseFilterExcludeDuplicateH('p')}
    ${purchaseFilterExcludeTokenWhenOrderExists('p')}
    GROUP BY ${variantExpr}
  `;
  const revSql = `
    SELECT ${variantExpr} AS variant_key,
           COALESCE(NULLIF(TRIM(p.order_currency),''), 'GBP') AS currency,
    ...
    ${purchaseFilterExcludeDuplicateH('p')}
    ${purchaseFilterExcludeTokenWhenOrderExists('p')}
    GROUP BY ${variantExpr}, COALESCE(NULLIF(TRIM(p.order_currency),''), 'GBP')
  `;
  ...
  for (const r of countRows || []) {
    const vk = r && r.variant_key != null ? String(r.variant_key) : 'other';
    byVariant.set(vk, {
      variantKey: vk,
      variantLabel: (r && r.variant_label != null ? String(r.variant_label) : '') || vk,
      channelKey: r && r.channel_key != null ? String(r.channel_key) : '',
      iconSpec: r && r.icon_spec != null ? String(r.icon_spec) : '',
      sessions: Number(r.sessions) || 0,
      orders: Number(r.orders) || 0,
      revenueGbp: 0,
    });
  ...
  return { ok: true, utmSource: utmSourceFilter, rows };
}
...
module.exports = {
  ...
  getOtherRevenueDrilldownByUtmSource,
};
