...
function getStats(req, res, next) {
  ...
  const inflight = reportCache.getOrComputeJson(
    ...
    () => {
      const t0 = Date.now();
      return store.getStats({ trafficMode, rangeKey }).then((data) => {
        const totalMs = Date.now() - t0;
        if (timing || totalMs > 2000) {
          console.log('[stats] range=%s ms_total=%s', rangeKey, totalMs);
        }
        return data;
      });
    }
    ...
    .then((r) => {
      if (timing) console.log('[stats] range=%s cacheHit=%s', rangeKey, r && r.cacheHit ? 1 : 0);
