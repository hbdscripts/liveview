...
async function getShopifyBestVariants(req, res) {
  ...
  try {
    ...
    const cached = await reportCache.getOrComputeJson(
      ...
      async () => {
        ...
        if (req.query && (req.query.timing === '1' || totalMs > 1500)) {
          console.log(
            '[shopify-best-variants] range=%s page=%s ms_total=%s ms_reconcile=%s ms_db_sessions=%s ms_db_count=%s ms_db_agg=%s ms_meta=%s',
            range,
            page,
            totalMs,
            msReconcile,
        ...
        return { bestVariants: pageItems, totalSessions, page, pageSize, totalCount };
      }
    );
    ...
    return res.json(cached && cached.ok ? cached.data : { bestVariants: [], totalSessions: 0, page: 1, pageSize, totalCount: 0 });
  } catch (err) {
