...
const TRACKER_TABLE_DEFINITIONS = [
  {
    ...
    columns: [
      ...
      { name: 'Source', value: 'sessions.attribution_source (write-time from deriveAttribution)' },
  ...
  },
  {
    ...
    requires: { dbTables: ['sessions', 'orders_shopify'], shopifyToken: false },
  },
  {
  ...
  {
    id: 'dashboard_overview_devices_chart',
    page: 'Overview',
    name: 'Devices chart (iOS/Android/Windows/Mac/Linux, 7 days)',
    ui: { elementIds: ['dash-chart-devices-30d'] },
    endpoint: { method: 'GET', path: '/api/devices/report', params: ['range=7d', 'force=1 (optional)'] },
    sources: [
      { kind: 'db', tables: ['sessions'], note: 'Human sessions grouped by ua_platform (derived from ingest User-Agent)' },
      { kind: 'db', tables: ['orders_shopify'], note: 'Paid truth orders grouped by device_key for orders/revenue breakdown' },
    ],
    columns: [
      { name: 'Segments', value: 'iOS | Android | Windows | Mac | Linux' },
      { name: 'Values', value: 'devices.rows[].platforms[].sessions (aggregated by platform)' },
    ],
    math: [],
    respectsReporting: { ordersSource: true, sessionsSource: true },
    requires: { dbTables: ['sessions', 'orders_shopify'], shopifyToken: false },
  ...
  {
    id: 'dashboard_overview_attribution_pie_chart',
    page: 'Overview',
    name: 'Attribution pie chart (30 days)',
    ui: { elementIds: ['dash-chart-attribution-30d'] },
    endpoint: { method: 'GET', path: '/api/attribution/report', params: ['range=30d', 'force=1 (optional)'] },
    sources: [
      { kind: 'db', tables: ['sessions'], note: 'Sessions grouped by attribution_channel (human sessions only)' },
      { kind: 'db', tables: ['orders_shopify'], note: 'Channel/source rollups also include orders + revenue in payload' },
    ],
    columns: [
      { name: 'Segments', value: 'Top channels by sessions in range' },
      { name: 'Values', value: 'attribution.rows[].sessions' },
    ],
    math: [],
    respectsReporting: { ordersSource: false, sessionsSource: true },
    requires: { dbTables: ['sessions'], shopifyToken: false },
  ...
  {
    id: 'insights_snapshot_page',
    page: 'Insights',
    name: 'Snapshot page + Profit Rules',
    ui: {
      elementIds: [
        'nav-tab-snapshot',
        'tab-panel-snapshot',
        'snapshot-preset-select',
        'snapshot-custom-start',
        'snapshot-custom-end',
        'snapshot-profit-settings-btn',
        'profit-rules-modal',
      ],
    },
    endpoint: {
      method: 'GET',
      path: '/api/business-snapshot',
      params: ['mode=range', 'since=YYYY-MM-DD', 'until=YYYY-MM-DD', 'preset=this_month|last_month|last_30_days|last_90_days|last_6_months|ytd|custom'],
    },
    sources: [
      ...
      { kind: 'shopify', note: 'Optional: Shopify Payments balance transactions for Transaction Fees and Shopify Fees cost groups (scope dependent; fails open, diagnostics included in response)' },
    ...
    columns: [
      { name: 'Financial', value: 'Revenue, Cost and optional Profit KPI table + chart overlay (this vs previous)' },
      { name: 'Performance', value: 'Sessions, Orders, Conversion Rate, AOV, optional Clicks/ROAS' },
      { name: 'Customers', value: 'New, Returning, Repeat Purchase Rate and LTV with customer trend chart' },
      { name: 'Profit Rules', value: 'Percent of Revenue, Fixed per Order, Fixed per Period with country targeting' },
      { name: 'Integrations', value: 'Profit Rules toggles: Google Ads spend (Integrations tab), Transaction Fees + Shopify Fees (Rules tab)' },
      { name: 'Charts', value: 'ApexCharts overlays for this vs previous period across each KPI group' },
    ...
    math: [
      ...
      { name: 'Rolling/custom comparison window', value: 'Compare against immediately preceding period with equal day length' },
      { name: 'Calendar comparison window', value: 'this_month/last_month compare against previous calendar month period' },
      { name: 'YTD comparison window', value: 'Compare Jan-01→today against Jan-01→same month/day in previous year' },
    ],
    respectsReporting: { ordersSource: false, sessionsSource: true },
    requires: { dbTables: ['settings', 'orders_shopify', 'orders_shopify_line_items'], shopifyToken: true },
  },
  ...
  },
  {
    ...
    columns: [
      ...
      { name: 'Source', value: 'sessions.attribution_source' },
  ...
  {
    id: 'acquisition_attribution_report',
    page: 'Acquisition',
    name: 'Attribution report',
    ui: { elementIds: ['attribution-table'] },
    endpoint: { method: 'GET', path: '/api/attribution/report', params: ['range=...', 'timezone/timeZone'] },
    sources: [
      { kind: 'db', tables: ['sessions'], note: 'Sessions grouped by attribution_source (write-time from deriveAttribution; human-only)' },
      { kind: 'db', tables: ['orders_shopify'], note: 'Orders/revenue from Shopify truth; attribution_source persisted at write-time' },
      { kind: 'db', tables: ['attribution_sources'], note: 'Labels/sort order for source keys (Settings → Acquisition)' },
      { kind: 'fx', note: 'Revenue converted to GBP' },
    ],
    columns: [
      { name: 'Sessions', value: 'COUNT(sessions) started_at in range (human-only)' },
      { name: 'Orders', value: 'orders', formula: 'COUNT(truth orders) in range for that source' },
      { name: 'Rev', value: 'revenueGbp', formula: 'Truth revenue in GBP' },
      { name: 'CR%', value: 'orders / sessions × 100' },
    ],
    math: [
      { name: 'Note', value: 'Orders/Rev are Shopify truth. Sessions use sessions.attribution_source (deriveAttribution at ingest).' },
      { name: 'Settings', value: 'Visible sources/labels from Settings → Acquisition (attribution_sources).' },
    ],
    respectsReporting: { ordersSource: false, sessionsSource: false },
    requires: { dbTables: ['sessions', 'attribution_sources'], shopifyToken: false },
  },
  {
    id: 'acquisition_devices_report',
    page: 'Acquisition',
    name: 'Devices report',
    ui: { elementIds: ['devices-table'] },
    endpoint: { method: 'GET', path: '/api/devices/report', params: ['range=...', 'timezone/timeZone'] },
    sources: [
      { kind: 'db', tables: ['sessions'], note: 'Sessions grouped by ua_device_type + ua_platform (human-only)' },
      { kind: 'db', tables: ['orders_shopify'], note: 'Orders/revenue from Shopify truth; device/platform from order client_details' },
      { kind: 'fx', note: 'Revenue converted to GBP' },
    ],
    columns: [
      { name: 'Sessions', value: 'COUNT(sessions) started_at in range (human-only)' },
    ...
    math: [
      { name: 'Note', value: 'Device/platform from ingest User-Agent and order payload.' },
    ],
    respectsReporting: { ordersSource: false, sessionsSource: false },
    requires: { dbTables: ['sessions'], shopifyToken: false },
  },
  ...
  {
    id: 'tools_change_pins',
    page: 'Tools',
    name: 'Change Pins tool',
    ui: { elementIds: ['tool-change-pins-create', 'tool-change-pins-list', 'tool-change-pins-detail'] },
    endpoint: {
      method: 'GET/POST/PATCH',
      path: '/api/tools/change-pins',
      params: [
        'GET: from_ymd/to_ymd/q/kind/include_archived/limit/offset',
        'POST: { event_ymd, event_hm?, title, kind?, magnitude_value?, magnitude_unit?, tags?, notes? }',
        'PATCH: /api/tools/change-pins/:id',
        'Effect: /api/tools/change-pins/:id/effect?window_days=7|14|30',
        'Recent: /api/tools/change-pins/recent?days=120',
      ],
    },
    sources: [
      { kind: 'db', tables: ['change_pins'], note: 'Persisted pins (annotations) with archive semantics (no hard deletes)' },
      { kind: 'fx', note: 'Effect uses store.getKpis() over before/after ranges for revenue/orders/sessions/CR/AOV/ROAS' },
    ],
    columns: [
      { name: 'Event date', value: 'change_pins.event_ymd (admin TZ date); optional event_ts for time-of-day markers' },
      { name: 'Title', value: 'change_pins.title' },
      { name: 'Kind', value: 'pricing|ads|site|ops|other' },
      { name: 'Tags', value: 'tags_json (array), tags_text (search)' },
      { name: 'Effect', value: 'Before vs after KPI comparison windows (7/14/30 days)' },
    ],
    math: [
      { name: 'Effect window', value: 'Before: r:(event-window):(event-1); After: r:event:(event+window-1) using admin timezone bounds (store.getRangeBounds).' },
      { name: 'Usage', value: 'Pins can be overlaid as vertical markers on dashboard time-series charts (read-only overlays).' },
    ],
    respectsReporting: { ordersSource: false, sessionsSource: false },
    requires: { dbTables: ['change_pins'], shopifyToken: false },
  },
  {
