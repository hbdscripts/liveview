...
- **Settings:** `/settings` (tabs: kexo, integrations, layout, attribution, insights, cost-expenses, admin). Kexo has General, Icons & assets, Payment Methods, Theme & display. Layout has Tables, KPIs, Date ranges only (chart settings via cog on each chart). Admin-only content gated by `isMasterRequest`.
...
- 2026-02-19: Payment Methods tab: new Kexo sub-tab at Settings → Kexo → Payment Methods. Lists all payment methods with built-in assets (Visa, Mastercard, Amex, PayPal, Klarna, Shop Pay, Apple Pay, Google Pay); each row shows icon + label + URL override input. Empty = use built-in; non-empty = override. Stored in `asset_overrides` as `payment_visa`, etc. Payment methods report API applies overrides. Payment-method glyph icons removed from Icons & Assets Misc (now only in Payment Methods tab).
