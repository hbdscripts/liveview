...
- **Number audit (source = our API `/api/stats`, human_only):**
  - **Sales** = order count + `getSalesTotal(start, end)` — displayed as "N  £X.XX" (N = orders in range). Source: purchases table (pixel checkout_completed events), GBP-converted; deduped by checkout_token/order_id or session+total+currency+15min. **If our count is lower than Shopify:** (1) some orders may not have sent checkout_completed to our pixel (thank-you page not loaded, or different flow); (2) some purchase rows may have been removed by dedupe (migration 016 or insert-time delete). Deleted rows are **not recoverable** without a DB backup. We only delete an h: row when inserting a token/order for the same session+total+currency within **2 min** (not 15 min) to avoid removing a different order.
